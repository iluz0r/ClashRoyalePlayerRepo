...
public class BattleStatusController{
	...
	protected boolean checkFirstCard() throws AWTException{
		...
		if(imageComparison.imgEqual(capturedCard, arrows, 15))
		{
			...
			if(imageComparison.imgEqual(capturedCard, babyDragon, 15))
			{
				success = true;
				this.firstCard = Card.BABY_DRAGON;
			} else
					if(imageComparison.imgEqual(capturedCard, balloon, 15))
					{
						success = true;
						this.firstCard = Card.BALLOON;
					} else
						if(imageComparison.imgEqual(capturedCard, valkyre, 15))
						{
							success = true;
							this.firstCard = Card.VALKYRE;
						} else
							if(imageComparison.imgEqual(capturedCard, eliteBarbarians, 15))
							{
								success = true;
								this.firstCard = Card.ELITE_BARBARIANS;
							} else
								if(imageComparison.imgEqual(capturedCard, fireSpirits, 15))
								{
									success = true;
									this.firstCard = Card.FIRE_SPIRITS;
								} else
									if(imageComparison.imgEqual(capturedCard, furnace, 15))
									{
										success = true;
										this.firstCard = Card.FURNACE;
									} else
										if(imageComparison.imgEqual(capturedCard, giant, 15))
										{
											success = true;
