...
public class BattleStatusController {
	...
	public BattleStatusController(AutoPlayer autoPlayer) throws Exception {
		...
		battleImageStore = new BattleImageStore(
				"." + File.separator + "images" + File.separator + "battle" + File.separator);
	}
	...
	protected void checkElisir() throws AWTException {
		...
		if (imageComparison.imgEqual(capturedElisir, tenElisir, 5)) {
			elisir = Elisir.TEN;
		} else if (imageComparison.imgEqual(capturedElisir, threeElisir, 5)) {
			elisir = Elisir.THREE;
		} else if (imageComparison.imgEqual(capturedElisir, nineElisir, 5)) {
			elisir = Elisir.NINE;
		} else if (imageComparison.imgEqual(capturedElisir, fourElisir, 5)) {
			elisir = Elisir.FOUR;
		} else if (imageComparison.imgEqual(capturedElisir, eightElisir, 5)) {
			elisir = Elisir.EIGHT;
		} else if (imageComparison.imgEqual(capturedElisir, fiveElisir, 5)) {
			elisir = Elisir.FIVE;
		} else if (imageComparison.imgEqual(capturedElisir, sevenElisir, 5)) {
			elisir = Elisir.SEVEN;
		} else if (imageComparison.imgEqual(capturedElisir, sixElisir, 5)) {
			elisir = Elisir.SIX;
		} else if (imageComparison.imgEqual(capturedElisir, threeElisir, 5)) {
			elisir = Elisir.THREE;
		} else if (imageComparison.imgEqual(capturedElisir, twoElisir, 5)) {
			elisir = Elisir.TWO;
		} else if (imageComparison.imgEqual(capturedElisir, oneElisir, 5)) {
			elisir = Elisir.ONE;
		} else if (imageComparison.imgEqual(zeroElisir, capturedElisir, 5)) {
			elisir = Elisir.ZERO;
		}
	...
	protected boolean checkFirstCard() throws AWTException {
		...
		BufferedImage valkyrie = battleImageStore.getValkyre();
		...
		if (imageComparison.imgEqual(capturedCard, arrows, 15)) {
			success = true;
			this.firstCard = Card.ARROWS;
		} else if (imageComparison.imgEqual(capturedCard, babyDragon, 15)) {
			success = true;
			this.firstCard = Card.BABY_DRAGON;
		} else if (imageComparison.imgEqual(capturedCard, balloon, 15)) {
			success = true;
			this.firstCard = Card.BALLOON;
		} else if (imageComparison.imgEqual(capturedCard, valkyrie, 15)) {
			success = true;
			this.firstCard = Card.VALKYRE;
		} else if (imageComparison.imgEqual(capturedCard, eliteBarbarians, 15)) {
			success = true;
			this.firstCard = Card.ELITE_BARBARIANS;
		} else if (imageComparison.imgEqual(capturedCard, fireSpirits, 15)) {
			success = true;
			this.firstCard = Card.FIRE_SPIRITS;
		} else if (imageComparison.imgEqual(capturedCard, furnace, 15)) {
			success = true;
			this.firstCard = Card.FURNACE;
		} else if (imageComparison.imgEqual(capturedCard, giant, 15)) {
			success = true;
	...
	protected boolean checkSecondCard() throws AWTException {
		...
		} else if (imageComparison.imgEqual(capturedCard, valkyre, 15)) {
			success = true;
	...
	protected boolean checkAllCards() throws AWTException {
		...
		if (Elisir.toInt(this.elisir) >= 5) {
			...
			if (Elisir.toInt(this.elisir) >= 6) {
				if (successFirstCard && successSecondCard && successThirdCard && successFourthCard) {
					success = true;
				}
			} else if (!successFirstCard && successSecondCard && successThirdCard && successFourthCard) {
				this.firstCard = Card.ELITE_BARBARIANS;
				success = true;
			} else if (successFirstCard && !successSecondCard && successThirdCard && successFourthCard) {
				this.secondCard = Card.ELITE_BARBARIANS;
	...
	protected boolean haveCard(Card card) {
		if (firstCard == card || secondCard == card || thirdCard == card || fourthCard == card) {
			return true;
		} else {
			return false;
		}
