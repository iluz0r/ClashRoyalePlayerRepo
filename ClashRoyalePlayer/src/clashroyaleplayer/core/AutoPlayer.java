...
public class AutoPlayer {
	...
	public AutoPlayer(Resolution screenResolution) throws Exception {
		String path = "." + File.separator + "log" + File.separator + LocalDate.now().toString() + "_"
				+ LocalTime.now().toString().replaceAll(":", "-") + ".log";
	...
	public void start() throws Exception {
		...
		while (gameController.gameStatus != GameStatus.UNKNOWN) {
			...
			case BATTLE_MENU: {
				...
				if (this.gameController.getFirstChestStatus() != GameStatusController.ChestStatus.UNLOCKING
					...
					if (this.gameController.getFirstChestStatus() == GameStatusController.ChestStatus.LOCKED) {
						...
						break;
					} else if (this.gameController.getSecondChestStatus() == GameStatusController.ChestStatus.LOCKED) {
						actions.unlockSecondChest();
						break;
					} else if (this.gameController.getThirdChestStatus() == GameStatusController.ChestStatus.LOCKED) {
						actions.unlockThirdChest();
						break;
					} else if (this.gameController.getFourthChestStatus() == GameStatusController.ChestStatus.LOCKED) {
	...
	private void playStrategie() throws AWTException {
		...
		case OFFENSIVE: {
			...
			} else if (battleController.haveCard(Card.ELITE_BARBARIANS) && battleController.haveCard(Card.VALKYRE)) {
				if (strategies.barbariansValkyre()) {
					switchScope = true;
				}
			} else if (battleController.haveCard(Card.BALLOON) && battleController.haveCard(Card.FIRE_SPIRITS)) {
				if (strategies.baloonSpirits()) {
					switchScope = true;
				}
			} else if (battleController.haveCard(Card.ELITE_BARBARIANS)
					&& battleController.haveCard(Card.FIRE_SPIRITS)) {
				if (strategies.barbariansSpirits()) {
					switchScope = true;
				}
			} else if (battleController.haveCard(Card.BALLOON) && battleController.haveCard(Card.ARROWS)) {
				if (strategies.balloonArrows()) {
					switchScope = true;
				}
			} else if (battleController.haveCard(Card.GIANT) && battleController.haveCard(Card.FIRE_SPIRITS)) {
				if (strategies.giantSpirits()) {
					switchScope = true;
				}
			} else if (battleController.haveCard(Card.ELITE_BARBARIANS)
					&& battleController.haveCard(Card.BABY_DRAGON)) {
				if (strategies.barbariansDragon()) {
					switchScope = true;
				}
			} else if (battleController.haveCard(Card.BALLOON)) {
				if (strategies.balloon()) {
					switchScope = true;
				}
			} else if (battleController.haveCard(Card.GIANT) && battleController.haveCard(Card.BABY_DRAGON)) {
				if (strategies.giantDragon()) {
					switchScope = true;
				}
			} else if (battleController.haveCard(Card.ELITE_BARBARIANS) && battleController.haveCard(Card.ARROWS)) {
				if (strategies.barbariansArrows()) {
					switchScope = true;
				}
			} else if (battleController.haveCard(Card.BALLOON) && battleController.haveCard(Card.BABY_DRAGON)) {
				if (strategies.balloonDragon()) {
					switchScope = true;
				}
			} else if (battleController.haveCard(Card.BALLOON) && battleController.haveCard(Card.GIANT)) {
				if (strategies.balloonGiant()) {
					switchScope = true;
				}
			} else if (battleController.haveCard(Card.VALKYRE) && battleController.haveCard(Card.BABY_DRAGON)) {
				if (strategies.valkyrieDragon()) {
					switchScope = true;
				}
			}
			break;
		...
		case DEFENSIVE: {
			...
			} else if (battleController.haveCard(Card.BABY_DRAGON)) {
				if (strategies.babyDragon()) {
					switchScope = true;
				}
			} else if (battleController.haveCard(Card.VALKYRE)) {
				if (strategies.valkyrie()) {
					switchScope = true;
				}
			} else if (battleController.haveCard(Card.ELITE_BARBARIANS) && battleController.haveCard(Card.VALKYRE)) {
				if (strategies.barbariansValkyre()) {
